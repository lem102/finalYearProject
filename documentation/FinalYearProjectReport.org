...
The product that I will produce at the end of this project is a
compiler (henceforth referred to as JPLcompiler) for a general purpose
programming language of my own creation. JPLcompiler will be able to
compile a program written in said programming language into assembly
language targeting the x86 CPU architecture. JPLcompiler will be
written in a modular, easily understandable way using the Java
programming language. It will be written in this way to encourage
learning about the various stages of compilation.

As a supplement to JPLcompiler I will also provide a learning plan in
pdf form to demonstrate how JPLcompiler could be used as a learning resource,
...
in terms of their education in computer science or programming.
...
we would want JPLcompiler to throw an error as dividing a word by a number
...
we wouldn't want test to be accessible outside of the if statement, as
...
would also be useful for the later parts of JPLcompiler where we are creating
...
use in later parts of JPLcompiler input's journey through JPLcompiler.

** Related Work

*** A Compiler for Teaching about Compilers

This paper sounds like it has a comparable spirit to this project in
that it espouses similar ideas regarding how the use of compiler
creation tools effect educational benefits, but the paper discusses a
compiler that is designed in order to teach a course, whereas mine is
simply a resource from which you can see how a compiler could be
implemented without the use of compiler creation tools
\citep{compilerForTeachingCompilers}.

The compiler in this course is designed to be very modular, so that a
student on the course could take out a component of the compiler and
replace it with their own. This also means that the student would be
able to replace parts of their own work with the teachers, which could
be useful if they wanted to see how that part of the compiler is
supposed to function.

This project and my project are similar in that they both involve
creating a compiler that needs to be modular and simple to understand
so that students can learn the basic concepts of how compilers work
from reading the provided source code. This means that both projects
will need to have clean, readable code.

The projects differ in their purpose, however. My project is simply a
supplement to an existing course, or perhaps just an example to
showcase the inputs and outputs of different steps of compilation. The
project described in the paper is meant to be at the centre of a
university module, and is designed to be extremely modular to the
point that students can remove some component and replace it with
their own. Although my compiler will try to be modular in order to
encourage experimentation, it is not the primary focus of my project,
which is to demonstrate that a compiler can be built without needing
lots of theoretical knowledge.

*** A Set Of Tools To Teach Compiler Construction

This paper introduces a set of tools to aid in the teaching of
compilers, as the authors of the paper found that some of the tools
commonly used in compiler construction were either obsolete or lacking
in terms of educational features. One example of how they remedied
this is by making use of a modified GNU bison, which outputs a
detailed description of the various states the parser is in whilst
parsing the input tokens. This information was lacking in the original
bison, making it very difficult to find errors in either the input or
the parser code.

My project differs from the tools described in the above paper quite
significantly.  In the paper, they still make use of tools to create
code which skips over the gory details. These tools are better for
education, which is an improvement, but I want to stick to just using
a single programming language (Java) in my project. My intention with
this is to reveal how a normal student could create a compiler with
out the use of complicated tools and theories, therefore making the
student totally understand the process of compilation
\citep{aSetOfToolsToTeachCompilerConstruction}.

*** Javacup

Javacup is a parser generator \citep{javacup}. You can use it to
generate a parser by giving it a set of rules. Javacup is a good
example of software used to teach compilation, indeed I used it during
my second year at Huddersfield University in the Language Translators
module. In my experience, it was good for understanding some of the
theory behind parsing, but didn't give me much of an idea of how to
implement my own parser from scratch.

Unfortunately, Javacup shares a common problem with parser generators
or "compiler compilers" in that it can be very hard to debug. This
resulted in me and other students wasting a significant amount of time
trying to find syntax errors we had made.

Javacup greatly differs from my project in most aspects; aside from
the fact that they are both meant to be used educationally. Javacup
also specifically covers the parsing stage of compilation, whereas
JPLcompiler provides opportunities for learning about all stages of
compilation.

** Professional, Legal Ethical \amp Social Issues

As a developer, I must be aware of and understand the Computer Misuse
Act 1990. This is important so that i can avoid accidentally
performing illegal acts when carrying out my work. Fortunately,
JPLcompiler does not involve any functionality that could break this
law.

As for Intellectual Property, JPLcompiler made use of the legal
concept known as "Fair Dealing" from the Copyright, Designs and
Patents Act 1988 during its creation and development as it is intended
to be used as a learning tool.

A professional issue that I face is the first item in the "Show what
you know, learn what you don't" section of the bcs code of conduct:
"only undertake to do work or provide a service that is within your
professional competence" \citep{bcs}. To resolve this issue I have
conducted research into the subject matter of compilation to ensure I
possess the skills to deliver a worthy product.

An ethical issue that my project faces is the possibility of the
compiler being used to create a program that is used in an unethical
way (e.g. compiling a program that is used to perform calculations to
assist in the development of weaponry). Unfortunately, there is
nothing I can do to prevent this.

My compiler will have a positive social effect, as it can be used in
the classroom to teach students. Achieving this is one of the goals of
the project.
...
would be JPLcompiler itself, so the main requirement of this project would
...
The language that JPLcompiler compiles from needs to be sufficiently complicated
...
should JPLcompiler be written in. I chose to use Java, but considered
two other programming languages, Python and C#.

I thought Python could be a good choice for JPLcompiler due to its prevalence
in education as well as its simpler syntax when compared to Java, which
...
in use at each stage of JPLcompiler. A statically typed language would
allow me to catch trivial bugs at compile time rather than having to waste
time debugging something breaking during run time.
...
I chose Java as the implementation language of JPLcompiler because it
...
features, such as polymorphism and inheritance, which allow for very productive
coding.
...
main risk to this project that I can foresee is running out of time before
the central deliverable (JPLcompiler) is completed. As I have no experience
...
is checked for correctness, so if a syntactical or semantic error is
found, then JPLcompiler will emit an error message. As this compiler is
...
I think I can split the development of JPLcompiler into four main stages,
each of which can be broken up into sub-stages:
...
In regards to how much time each stage of JPLcompiler will take to write,
...
Now I will discuss the design of JPLcompiler.

** Product Specification

When thinking about how I was going to design JPLcompiler, I had to consider
...
As we saw in the "structure of the compiler" subsection earlier, the process
...
would be able to alter the source code that JPLcompiler is compiling from,
...
outputs of each stage \citep[p.~7]{dragon}:
...
To implement JPLcompiler, I had to make use of several different skills
and bits of knowledge. In order to create anything, I first had to be able
to program. Programming requires knowledge of a programming language (Java)
...
to be run to allow for JPLcompiler to be run, I wrote a shell script which
required knowledge of the bash scripting language. 

When writing JPLcompiler, I needed to know about the structure of a compiler
and the various transformations source code goes through during its journey
through JPLcompiler, detailed previously in this report. Additionally,
I will need to learn at least the basics of assembly language in order
to write the code generator part of JPLcompiler.

Another thing I needed to be aware of is the cleanliness of my code when
writing JPLcompiler. Clean code means that it is easy to understand. Code
...
In the JPL language, there are expressions in a similar manner to
C-like languages. For example, the following expression will evaluate
to 14, which will be stored in the variable number:
...
to make sure that the operations are evaluated in an order that
respects operator precedence. First, it will search for the "root
element" of the expression. For example, in the following:
...
types. This array is then iterated over in a foreach loop to see if a
type is present in the provided tokens and returns its position in the
list of tokens if it is, exiting the method.

The order of the array is important, the operations with the greatest
precedence are located towards the end of the array. This means that
when the intermediate representation is generated, the operators with
greater precedence are evaluated first, as they are closest to the
furthest away leaves of the tree created by the expression.  For
example:
...
of a custom Java object, referred to in JPLcompiler code as a IntermediateCodeInstruction.
...
is based on the quadruple concept discussed in the Dragon book \citep[p.~366]{dragon}.
...
requirements of JPLcompiler will be clearly different to what it should
...
with a source code file as an argument to run JPLcompiler. 

* Evaluation
** Product Evaluation

My initial strategy for evaluating the product was to get second year
university students to use the product and record any comments they
would make regarding its usefulness in learning.

Unfortunately, due to the global Coronavirus pandemic I have been
unable to do this. Instead, I have opted to get a couple of my fellow
final year students to use the product under my supervision via a
Microsoft Teams call. I have recorded these calls and received useful
feedback from both of them.

The feedback I received can mostly be grouped into two groups. The
first feedback group concerns the usability of the product. One piece
of feedback I received was "It would be difficult for me to figure out
how to use the product by myself." another piece of feedback I
received was "The syntax tree is very difficult to read." from this
feedback I realised that the product is somewhat user unfriendly. To
rectify this I could add a better, more detailed, help message. To
resolve the parser readability issue I could change the output of the
parser to look more like a tree.

The second feedback group concerns problems with JPLcompiler
itself. I received feedback in which more datatypes were requested,
and to allow for negative integers. I could have added more data types
but chose not to in order to focus on assembly generation; however I
have made it easy to extend JPLcompiler to add these datatypes.

A critical problem was discovered where if you have a parameter that
is the same name as a variable, the assembly generator will create
assembly which has two variables of the same name, which is illegal. I
could fix this using a similar method to how i name temporary
variables, this would avoid the namespace collision.
...
JPLcompiler can be used to write programs that make use of variables,
flow control (such as if statements and while loops) and functions which
take parameters. 

The only data type that can be handled by JPLcompiler is a non-negative
...
Another requirement of JPLcompiler, is to emit readable output to the
...
JPLcompiler meets this requirement by being able to output the results
...
*** Flaws of JPLcompiler

- Parameter Namespace Collision

The biggest problem with JPLcompiler can be observed if, somewhere in
...
JPLcompiler will make two variables of the same name. This causes illegal
...
In JPLcompiler there is only one data type, an unsigned integer. There
...
JPLcompiler uses temporary variables to hold the results of parts of expressions.
...
the java compiler for files that haven't been compiled before or changed
...
JVM (Java Virtual Machine) or the CLR (Common Language Runtime). And of
...
of acquiring an understanding of how to write programs in assembly. This
...
hard drive failure or power cuts, I made use of the git version control
...
advanced algorithm for generating efficient machine code. A commercial
compiler would be able to reduce the number of instructions required to
create equivalent but faster assembly. A commercial compiler would also
...
More data types would allow for more useful programs to be created, especially
...
and complex topics such as type systems to be explored within JPLcompiler.
...
as input and creates a graphical representation of the syntax tree. I would
like to add this feature to JPLcompiler because unlike the other outputs
of JPLcompiler, the text representation of the syntax tree is very difficult
...
generator so that the assembly code generated makes more efficient use
